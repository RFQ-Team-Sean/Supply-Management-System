...
export class UserFilterComponent implements OnChanges {
  ...
  // Activity Logs filter properties
  actionTypes = ['Login', 'Create', 'Approve PR', 'Reject PR', 'General Report'];
  ...
  applyFilters() {
    if (this.currentView === 'users') {
      ...
      this.filterChanged.emit(this.logsFilter);
    }
  }

  generateReport() {
    if (!this.logsFilter.exportType) {
      alert('Please select an export type');
      return;
    }
    
    // Emit the filter data for report generation
    this.filterChanged.emit({
      ...this.logsFilter,
      action: 'generate'
    });
